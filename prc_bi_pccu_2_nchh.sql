CREATE DEFINER=`root`@`%` PROCEDURE `prc_bi_pccu_2_nchh`()
BEGIN
...
IF cur_dow IN (5,6,7) OR cur_date BETWEEN '2023-01-17' AND '2023-01-20' THEN
...
CASE 
	WHEN tb.Kho_giao_hang = N'01.TỔNG KHO 2' 
		...
		AND tb.goi_y_dat_hang > tb.MOQ_TO 
			THEN CEILING(tb.goi_y_dat_hang/tb.MOQ_TO)*tb.MOQ_TO
	WHEN tb.Kho_giao_hang = N'01.TỔNG KHO 2' 
		AND tb.MOQ_TO <= 0
		AND tb.goi_y_dat_hang > tb.MOQ_TO
