CREATE DEFINER=`root`@`%` PROCEDURE `prc_bi_pccu_2_dat_hang`()
BEGIN

/*--------------------------------------------------!
01. Đặt hàng kho tổng
!--------------------------------------------------*/

TRUNCATE TABLE data_kiot_viet.dat_hang_ver2;
INSERT INTO data_kiot_viet.dat_hang_ver2
...
oh_kh AS
(
SELECT SUM(onhand) onhand, productcode
FROM
    data_kiot_viet.inventory_daily
WHERE
    DATE(reportingdate) = CURDATE()
        AND branchid IN ('116012','1000000065')
GROUP BY productcode
),
oh_ch AS
(
...
WHERE
    DATE(reportingdate) = CURDATE()
        AND branchid NOT IN ('116012','1000000065','253685')
GROUP BY productcode
),
tf_to_tk AS
(
...
cost_amt AS 
(
SELECT cost, productcode FROM  data_kiot_viet.inventory_daily WHERE branchname = N'01.TỔNG KHO 2' AND DATE(reportingdate) = CURDATE()
),
dat_hang AS (
SELECT DISTINCT 
nchh.nv_cu,
...
IFNULL(oh_ch.onhand,0) + IFNULL(oh_tk.onhand_tk,0) - IFNULL(oh_kh.onhand,0) tk_hethong -- đổi lại đkien 30.01.2023
...
LEFT JOIN oh_ch
ON nchh.productcode = oh_ch.productcode
LEFT JOIN oh_kh
ON nchh.productcode = oh_kh.productcode
...
/*--------------------------------------------------!
02. Đặt hàng cửa hàng
!--------------------------------------------------*/

TRUNCATE TABLE data_kiot_viet.dat_hang_ch_ver2;
INSERT INTO data_kiot_viet.dat_hang_ch_ver2
SELECT 
    nchh.event_date,
    nchh.productcode,
    nchh.productname,
