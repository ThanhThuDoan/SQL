CREATE DEFINER=`root`@`%` PROCEDURE `prc_bi_pkd_1_target_actual_prv`()
BEGIN
...
WHERE MONTH(date_field) IN (MONTH(NOW() - INTERVAL 1 MONTH))
    AND (
          YEAR(date_field) = YEAR(NOW()) 
        OR 
          YEAR(date_field) = YEAR(NOW() - INTERVAL 1 MONTH))
...
/*
DROP VIEW data_kiot_viet.daily_sales_asm_rp_prv_v;
...
WHERE
    m_rp IN (DATE_FORMAT(DATE(NOW() - INTERVAL 2 MONTH), '%Y-%m') , DATE_FORMAT(DATE(NOW() - INTERVAL 1 MONTH), '%Y-%m'))
) ds_br
...
INNER JOIN data_kiot_viet.master_sales_target_prvm_temp tar ON rs_ds_br.branchname = tar.cua_hang AND rs_ds_br.createdDate = tar.date_field
) rs_ds_br1
...
/*--------------------------------------------------!
Báo cáo So sánh cùng kỳ MTD - Mục tiêu thực đạt
!--------------------------------------------------*/

TRUNCATE TABLE data_kiot_viet.pkd_so_sanh_cungky_week;

INSERT INTO data_kiot_viet.pkd_so_sanh_cungky_week
SELECT
...
SUM(Slg_SKU) slg_SKU,
SUM(subtotal_f) subtotal_f
FROM data_kiot_viet.daily_sales_asm_rp_v 
...
SUM(Slg_SKU) slg_SKU,
SUM(subtotal_f) subtotal_f
FROM data_kiot_viet.daily_sales_asm_rp_prv_v 
