CREATE DEFINER=`root`@`%` PROCEDURE `prc_daily_sales_branch_all`()
BEGIN
...
WHERE YEAR(inv_dt.createdDate) =  '2022'-- YEAR(DATE_ADD(CURDATE(), INTERVAL -1 DAY))
AND inv.status <> 2
) rs
WHERE customercode = ''
...
WHERE
    rt.status = 1
        AND YEAR(rt_dt.createdDate) = '2022'#YEAR(DATE_ADD(CURDATE(), INTERVAL -1 DAY))
...
WHERE YEAR(createdDate) = '2022'#YEAR(DATE_ADD(CURDATE(), INTERVAL -1 DAY))
GROUP BY createdDate, 
m_rp, 
y_rp, 
branchid, 
