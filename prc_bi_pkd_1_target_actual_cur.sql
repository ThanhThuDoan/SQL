CREATE DEFINER=`root`@`%` PROCEDURE `prc_bi_pkd_1_target_actual_cur`()
BEGIN

SET SQL_MODE = 'NO_ENGINE_SUBSTITUTION';
SET SQL_SAFE_UPDATES = 0;

TRUNCATE TABLE data_kiot_viet.daily_sales_branch_daily;
INSERT INTO data_kiot_viet.daily_sales_branch_daily
...
FROM
    data_kiot_viet.invoices_details_daily inv_dt INNER JOIN 
    data_kiot_viet.invoices_daily inv
    ON  inv_dt.invoiceid = inv.id
WHERE DATE(inv_dt.createdDate) = CURDATE()
AND inv.status <> 2
) rs
WHERE customercode = ''
...
FROM
    data_kiot_viet.returns_details_daily rt_dt
        INNER JOIN
    data_kiot_viet.returns_daily rt ON rt_dt.code = rt.code
WHERE DATE(rt_dt.createdDate) = CURDATE()
    AND rt.status = 1
