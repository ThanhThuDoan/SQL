CREATE DEFINER=`root`@`%` PROCEDURE `prc_kpi_thumua_cungung`()
BEGIN
...
FROM data_kiot_viet.inventory 
WHERE YEAR(DATE_ADD(DATE(reportingDate), INTERVAL -1 DAY)) = YEAR(DATE_ADD(CURDATE(), INTERVAL -1 DAY))
AND MONTH(DATE_ADD(DATE(reportingDate), INTERVAL -1 DAY)) = MONTH(DATE_ADD(CURDATE(), INTERVAL -1 DAY))
...
WHERE YEAR(DATE_ADD(DATE(date_field), INTERVAL -1 DAY)) = YEAR(DATE_ADD(CURDATE(), INTERVAL -1 DAY))
AND MONTH(DATE_ADD(DATE(date_field), INTERVAL -1 DAY)) = MONTH(DATE_ADD(CURDATE(), INTERVAL -1 DAY))
),
dmhh AS (
...
ton_ko AS (
SELECT invt_cur.reportingDate, invt_cur.productcode, COUNT(DISTINCT invt_cur.branchname) slg_ch_ton_0 FROM data_kiot_viet.inventory_curmonth_tk invt_cur
WHERE invt_cur.branchname <> '01.TỔNG KHO 2'
AND invt_cur.onhand = 0
GROUP BY invt_cur.reportingDate, invt_cur.productcode
),
ds AS (
SELECT createdDate,productcode, SUM(subtotal) subtotal FROM data_kiot_viet.daily_sales 
WHERE YEAR(createdDate) = YEAR(DATE_ADD(CURDATE(), INTERVAL -1 DAY))
AND MONTH(createdDate) = MONTH(DATE_ADD(CURDATE(), INTERVAL -1 DAY))
