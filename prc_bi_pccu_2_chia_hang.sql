CREATE DEFINER=`root`@`%` PROCEDURE `prc_bi_pccu_2_chia_hang`()
BEGIN
...
TRUNCATE TABLE data_kiot_viet.chia_hang_ver2;
INSERT INTO data_kiot_viet.chia_hang_ver2
WITH list_ch1 AS (
...
CASE 
	WHEN nchh.onhand = 0 
		THEN MAX(nchh.quantity_f) OVER(PARTITION BY nchh.productcode) 
			ELSE quantity_f  
END rank_quantity
FROM data_kiot_viet.order_nchh_ver2 nchh
LEFT JOIN (
SELECT SUM(onhand) onhand_tk, productcode
FROM (SELECT DISTINCT invt.* FROM
    data_kiot_viet.inventory_daily invt
WHERE
...
/*TRUNCATE TABLE data_kiot_viet.inventory_exp_tk_ver2;
INSERT INTO data_kiot_viet.inventory_exp_tk_ver2
SELECT * FROM data_kiot_viet.inventory_exp
WHERE branchID = '253685' AND onhand <> 0;
COMMIT;*/

TRUNCATE TABLE data_kiot_viet.chia_hang_f_ver2;
INSERT INTO data_kiot_viet.chia_hang_f_ver2
SELECT 
    ...
    ch.loai_don
FROM
    data_kiot_viet.chia_hang_ver2 ch WHERE
...
FROM
    data_kiot_viet.chia_hang_ver2 ch
WHERE final_chia <> 0 
#AND EXISTS (SELECT NULL FROM data_kiot_viet.inventory_exp_tk_ver2 exp WHERE ch.productid = exp.productId)
...
TRUNCATE TABLE data_kiot_viet.exp_temp_ver2;
INSERT INTO data_kiot_viet.exp_temp_ver2
SELECT id,
    productId,
    batchname,
...
FROM
    data_kiot_viet.inventory_exp_tk_ver2 WHERE onhand <> 0
AND productid = prd1;
...
SELECT * FROM  data_kiot_viet.exp_temp_ver2 WHERE onhand <> 0 ORDER BY rn;
...
SELECT 
    ...
    ch.nv_cu,
    ch.onhand,
    exp_temp_ver2.onhand onhand_tk,
    ch.rank_quantity,
    ch.row_num,
    ch.flag_1,
    ch.flag_2,
    ch.final_chia,
    exp_temp_ver2.batchname,
    exp_temp_ver2.hsd,
    ch.loai_don
FROM
    data_kiot_viet.chia_hang_ver2 ch
        INNER JOIN
    data_kiot_viet.exp_temp_ver2 ON ch.productid = exp_temp_ver2.productid
    WHERE ch.productid = prd1
    AND ch.branchname = branch
    AND exp_temp_ver2.batchname = batn
    AND exp_temp_ver2.id = idb;
COMMIT;
UPDATE data_kiot_viet.inventory_exp_tk_ver2 
SET 
    onhand = 0
WHERE
    id = idb AND batchname = batn;
COMMIT;
UPDATE data_kiot_viet.chia_hang_ver2 
SET 
    final_chia = 0
WHERE
    productid = prd1 AND branchname = branch;
COMMIT;
...
SELECT 
    ...
    ch.flag_1,
    ch.flag_2,
    exp_temp_ver2.onhand final_chia,
    exp_temp_ver2.batchname,
    exp_temp_ver2.hsd,
    ch.loai_don
FROM
