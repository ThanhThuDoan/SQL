CREATE DEFINER=`root`@`%` PROCEDURE `prc_daily_sales_branch`()
BEGIN
...
/*--------------------------------------------------!
02. Get data doanh thu & return theo cửa hàng
!--------------------------------------------------*/

TRUNCATE TABLE data_kiot_viet.daily_sales_branch;

INSERT INTO data_kiot_viet.daily_sales_branch
...
WHERE DATE(inv_dt.createdDate) BETWEEN DATE_ADD(DATE_ADD(CURDATE(), INTERVAL - 1 DAY),
...
WHERE
    rt.status = 1
        AND DATE(rt_dt.createdDate) BETWEEN DATE_ADD(DATE_ADD(CURDATE(), INTERVAL - 1 DAY),
        INTERVAL - DAY(DATE_ADD(CURDATE(), INTERVAL - 1 DAY)) + 1 DAY) AND LAST_DAY(DATE_ADD(CURDATE(), INTERVAL - 1 DAY))
...
FROM 
(
SELECT * FROM data_kiot_viet.daily_sales_branch
UNION ALL
